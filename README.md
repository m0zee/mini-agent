...
![Skill-matching flow: from a user prompt through tier-1 catalog disclosure, the model's activation decision, tier-2 SKILL.md loading, to the final answer](docs/architecture-flow.svg)
...
Every path below is a real file in this repo.
...
Validation and catalog rendering deliberately mirror the behavior of the official `skills-ref` reference library described in the spec sources, and several of its documented test cases (name/description boundary conditions, Unicode-name handling, escaping) were ported directly into `test/parse.test.ts` and `test/catalog.test.ts`.
...
**Accepted, deliberate limitations** (found and left as-is during the build, each verified rather than merely asserted):
...
- ~~Deny-listed filenames (`.env`, `id_rsa`, etc.) are still visible in `list_directory` output~~ — fixed post-T12: `list_directory` now filters out any entry `fs-guard.ts`'s deny-list would also refuse to read (see Security above), so a listing no longer reveals that a secrets file exists at all, not just that its content is unreadable.
...
_Time spent: Planning and the initial implementation happened over a weekend, across multiple ~5-hour sessions. The following working days added further time for testing, review, and refinements (security hardening, live verification against the real API, small usability fixes)._
